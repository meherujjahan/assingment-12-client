...
const ManageAllProducts = () => {
    ...
    useEffect(() => {
        const url = 'https://warm-hollows-25705.herokuapp.com/purch'
        fetch(url)
            .then(res => res.json())
            .then(data => {
                setAllOrder(data)
    ...
    const handleProductDElt = id => {
        ...
        if (proceed) {
            const url = `https://warm-hollows-25705.herokuapp.com/purch/${id}`
            console.log(url)
            fetch(url, {
                method: 'DELETE',
            })
    ...
    const handleUpdate = (_id) => {
        ...
        if (confirm) {
            fetch(`https://warm-hollows-25705.herokuapp.com/purch/${_id}`, {
                method: "put",
            })
                .then(res => res.json())
                .then(data => {
